...
# snippets tarball, this script currently refrains from some
# formatting improvements.  However, it already contains lines marked
# with 'XXX' that should be deleted or added (as described) after the
# diffing process has been performed.
...
# XXX Unify this with `DOC_POSTAMBLE` to use consistent vertical
#     space.
use constant DOC_POSTAMBLE_NEW => <<~'EOT';
  } % begin verbatim
...
for my $entry (sort { $a->{"filename"} cmp $b->{"filename"} }
  ...
  if ($is_docs && $entry->{"approved"} == APPROVED) {
    ...
    if (($no_lsr && $version) || !$no_lsr) {
      ...
      $data .= $version ? DOC_POSTAMBLE_NEW : DOC_POSTAMBLE;
