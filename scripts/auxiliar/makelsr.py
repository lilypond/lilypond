...
#
# Copyright (C) 2007--2022 Graham Percival <graham@percival-music.ca>
#
...
def read_source(src):
    ...
    for f in glob.glob(os.path.join(src, '*.ly')):
        ...
        m = tags_re.search(open(f, 'r', encoding='utf-8').read())
        if m:
...
if in_dir:
    ...
    for f in glob.glob(os.path.join(lys_from_lsr, '*.ly')):
        if new_lys_marker in open(f, encoding='utf-8').read():
            os.remove(f)
snippets = {}
if in_dir:
