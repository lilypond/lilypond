...
def conv(str):
    str = re.sub('note[hH]eadStyle\\s*=\\s*"?(\\w+)"?',
                 "noteHeadStyle = #'\\1", str)
    return str
...
def conv(str):
    ...
    str = re.sub('\\\\property *[^ ]*verticalDirection[^=]*= *#?"?(1|(\\\\up))"?',
                 '\\\\stemUp\\\\slurUp\\\\tieUp', str)
    str = re.sub('\\\\property *[^ ]*verticalDirection[^=]*= *#?"?((-1)|(\\\\down))"?',
                 '\\\\stemDown\\\\slurDown\\\\tieDown', str)
    str = re.sub('\\\\property *[^ ]*verticalDirection[^=]*= *#?"?(0|(\\\\center))"?',
                 '\\\\stemBoth\\\\slurBoth\\\\tieBoth', str)

    str = re.sub('verticalDirection[^=]*= *#?"?(1|(\\\\up))"?',
                 'Stem \\\\override #\'direction = #0\nSlur \\\\override #\'direction = #0\n Tie \\\\override #\'direction = #1', str)
    str = re.sub('verticalDirection[^=]*= *#?"?((-1)|(\\\\down))"?',
                 'Stem \\\\override #\'direction = #0\nSlur \\\\override #\'direction = #0\n Tie \\\\override #\'direction = #-1', str)
    str = re.sub('verticalDirection[^=]*= *#?"?(0|(\\\\center))"?',
                 'Stem \\\\override #\'direction = #0\nSlur \\\\override #\'direction = #0\n Tie \\\\override #\'direction = #0', str)

    str = re.sub(
        '\\\\property *[^ .]*[.]?([a-z]+)VerticalDirection[^=]*= *#?"?(1|(\\\\up))"?', '\\\\\\1Up', str)
    str = re.sub(
        '\\\\property *[^ .]*[.]?([a-z]+)VerticalDirection[^=]*= *#?"?((-1)|(\\\\down))"?', '\\\\\\1Down', str)
    str = re.sub(
        '\\\\property *[^ .]*[.]?([a-z]+)VerticalDirection[^=]*= *#?"?(0|(\\\\center))"?', '\\\\\\1Both', str)

    # (lacks capitalization slur -> Slur)
    str = re.sub('([a-z]+)VerticalDirection[^=]*= *#?"?(1|(\\\\up))"?',
                 '\\1 \\\\override #\'direction = #1', str)
    str = re.sub('([a-z]+)VerticalDirection[^=]*= *#?"?((-1)|(\\\\down))"?',
                 '\\1 \\override #\'direction = #-1', str)
    str = re.sub('([a-z]+)VerticalDirection[^=]*= *#?"?(0|(\\\\center))"?',
                 '\\1 \\\\override #\'direction = #0', str)

    # dynamic..
    str = re.sub(
        '\\\\property *[^ .]*[.]?dynamicDirection[^=]*= *#?"?(1|(\\\\up))"?', '\\\\dynamicUp', str)
    str = re.sub(
        '\\\\property *[^ .]*[.]?dyn[^=]*= *#?"?((-1)|(\\\\down))"?', '\\\\dynamicDown', str)
    str = re.sub(
        '\\\\property *[^ .]*[.]?dyn[^=]*= *#?"?(0|(\\\\center))"?', '\\\\dynamicBoth', str)

    str = re.sub(
        '\\\\property *[^ .]*[.]?([a-z]+)Dash[^=]*= *#?"?(0|(""))"?', '\\\\\\1NoDots', str)
    str = re.sub(
        '\\\\property *[^ .]*[.]?([a-z]+)Dash[^=]*= *#?"?([1-9]+)"?', '\\\\\\1Dotted', str)

    str = re.sub(
        '\\\\property *[^ .]*[.]?noAutoBeaming[^=]*= *#?"?(0|(""))"?', '\\\\autoBeamOn', str)
    str = re.sub(
        '\\\\property *[^ .]*[.]?noAutoBeaming[^=]*= *#?"?([1-9]+)"?', '\\\\autoBeamOff', str)
    return str
...
def conv(str):
    str = re.sub('\\\\property *"?Voice"? *[.] *"?textStyle"? *= *"([^"]*)"',
                 '\\\\property Voice.TextScript \\\\set #\'font-style = #\'\\1', str)
    str = re.sub('\\\\property *"?Lyrics"? *[.] *"?textStyle"? *= *"([^"]*)"',
                 '\\\\property Lyrics.LyricText \\\\set #\'font-style = #\'\\1', str)

    str = re.sub('\\\\property *"?([^.]+)"? *[.] *"?timeSignatureStyle"? *= *"([^"]*)"',
                 '\\\\property \\1.TimeSignature \\\\override #\'style = #\'\\2', str)

    str = re.sub('"?timeSignatureStyle"? *= *#?""',
                 'TimeSignature \\\\override #\'style = ##f', str)

    str = re.sub('"?timeSignatureStyle"? *= *#?"([^"]*)"',
                 'TimeSignature \\\\override #\'style = #\'\\1', str)

    str = re.sub('#\'style *= #*"([^"])"', '#\'style = #\'\\1', str)

    str = re.sub('\\\\property *"?([^.]+)"? *[.] *"?horizontalNoteShift"? *= *"?#?([-0-9]+)"?',
                 '\\\\property \\1.NoteColumn \\\\override #\'horizontal-shift = #\\2', str)

    # ugh
    str = re.sub('\\\\property *"?([^.]+)"? *[.] *"?flagStyle"? *= *""',
                 '\\\\property \\1.Stem \\\\override #\'flag-style = ##f', str)

    str = re.sub('\\\\property *"?([^.]+)"? *[.] *"?flagStyle"? *= *"([^"]*)"',
                 '\\\\property \\1.Stem \\\\override #\'flag-style = #\'\\2', str)
    return str


@rule((1, 3, 102), 'beamAutoEnd -> autoBeamSettings \\push (end * * * *)')
def conv(str):
    str = re.sub('"?beamAutoEnd_([0-9]*)"? *= *(#\\([^)]*\\))',
                 'autoBeamSettings \\push #\'(end 1 \\1 * *) = \\2', str)
    str = re.sub('"?beamAutoBegin_([0-9]*)"? *= *(#\\([^)]*\))',
                 'autoBeamSettings \\push #\'(begin 1 \\1 * *) = \\2', str)
    str = re.sub('"?beamAutoEnd"? *= *(#\\([^)]*\\))',
                 'autoBeamSettings \\push #\'(end * * * *) = \\1', str)
    str = re.sub('"?beamAutoBegin"? *= *(#\\([^)]*\\))',
                 'autoBeamSettings \\push #\'(begin * * * *) = \\1', str)
    return str
...
def conv(str):
    str = re.sub('point-and-click +#t',
                 'point-and-click line-column-location', str)
    return str
...
def conv(str):
    str = re.sub('([A-Za-z]+)MinimumVerticalExtent',
                 'MinimumV@rticalExtent', str)
...
def conv(str):
    str = re.sub('textNonEmpty *= *##t',
                 "TextScript \\set #'no-spacing-rods = ##f", str)
    str = re.sub('textNonEmpty *= *##f',
                 "TextScript \\set #'no-spacing-rods = ##t", str)
    return str
...
def conv(str):
    ...
    str = re.sub(r"""Stem([ ]+)\\override #'style""",
                 r"""Stem \\override #'flag-style""", str)
    str = re.sub(r"""Stem([ ]+)\\set([ ]+)#'style""",
                 r"""Stem \\set #'flag-style""", str)
    return str
...
def conv(str):
    str = re.sub(
        '\\(ly-make-music *\"([A-Z][a-z_]+)_req\"\\)', subst_req_name, str)
...
def conv(str):
    ...
    str = re.sub(
        r'\\spanrequest *([^ ]+) *"([^"]+)"', subst_inline_ev_name, str)
    str = re.sub(
        r' *= *\\commandspanrequest *([^ ]+) *"([^"]+)"', subst_csp_definition, str)
    str = re.sub(
        r'\\commandspanrequest *([^ ]+) *"([^"]+)"', subst_csp_inline, str)
    str = re.sub(r'ly-id ', 'ly-import ', str)

    str = re.sub(r' *= *\\script "([^"]+)"',
                 ' = #(make-articulation "\\1")', str)
    str = re.sub(r'\\script "([^"]+)"',
                 '#(ly-export (make-articulation "\\1"))', str)
    return str
...
def conv(str):
    ...
    str = re.sub(r"ly:combine-molecule-at-edge",
                 "ly:molecule-combine-at-edge", str)
...
def conv(str):
    str = re.sub(
        r"linewidth *= *-[0-9.]+ *(\\mm|\\cm|\\in|\\pt)?", 'raggedright = ##t', str)
    return str
...
def conv(str):
    ...
    str = re.sub(
        r"\\property *Staff\.(Sustain|Sostenuto|UnaCorda)Pedal *\\revert *#'pedal-type", '', str)
    return str
...
def conv(str):
    ...
    str = re.sub(
        r"\\property ([a-zA-Z]+)\s*\.\s*automaticMelismata\s*=\s*##([ft])", func, str)
    return str
...
def conv(str):
    ...
    str = re.sub(r'\\defaultAccidentals',
                 "#(set-accidental-style 'default)", str)
    str = re.sub(r'\\voiceAccidentals', "#(set-accidental-style 'voice)", str)
    str = re.sub(r'\\modernAccidentals',
                 "#(set-accidental-style 'modern)", str)
    str = re.sub(r'\\modernCautionaries',
                 "#(set-accidental-style 'modern-cautionary)", str)
    str = re.sub(r'\\modernVoiceAccidental',
                 "#(set-accidental-style 'modern-voice)", str)
    str = re.sub(r'\\modernVoiceCautionaries',
                 "#(set-accidental-style 'modern-voice-cautionary)", str)
    str = re.sub(r'\\pianoAccidentals', "#(set-accidental-style 'piano)", str)
    str = re.sub(r'\\pianoCautionaries',
                 "#(set-accidental-style 'piano-cautionary)", str)
    str = re.sub(r'\\forgetAccidentals',
                 "#(set-accidental-style 'forget)", str)
...
def conv(str):
    ...
    str = re.sub(r"\\override\s+.*Arpeggio\s+#.print-function\s+=\s+\\arpeggioBracket",
                 r"\\arpeggioBracket", str)
    return str
...
def conv(str):
    str = re.sub(
        r'(slur|stem|phrasingSlur|tie|dynamic|dots|tuplet|arpeggio|)Both', r'\1Neutral', str)
...
'''))
    ...
    str = re.sub(r'#\(paper-set-staff-size',
...
def conv(str):
    ...
    str = re.sub(
        "'(vaticana|hufnagel|medicaea|petrucci|neomensural|mensural)-", r"'\1.", str)
    return str
...
def conv(str):
    str = re.sub(
        r'\\context\s+\"?([a-zA-Z]+)\"?\s*\\applyOutput', r"\\applyOutput #'\1", str)
    return str
...
""" % (num*count, den)
    ...
    str = re.sub(
        r'\\midi\s*{\s*\\tempo ([0-9]+)\s*([.]*)\s*=\s*([0-9]+)\s*}', sub_tempo, str)
    return str
...
def conv(str):
    str = re.sub(r"no-spacing-rods\s+=\s+##t",
                 r"extra-spacing-width = #'(+inf.0 . -inf.0)", str)
    str = re.sub(r"no-spacing-rods\s+=\s+##f",
                 r"extra-spacing-width = #'(0 . 0)", str)
    return str
...
def conv(str):
    ...
    str = re.sub(
        r"(\\once)?\s*\\override\s*([a-zA-Z]+\s*[.]\s*)?TextSpanner\s*#'edge-height\s*=\s*#'\(\s*([0-9.-]+)\s+[.]\s+([0-9.-]+)\s*\)", sub_edge_height, str)
...
def conv(str):
    str = re.sub(r"infinite-spacing-height\s+=\s+##t",
                 r"extra-spacing-height = #'(-inf.0 . +inf.0)", str)
    str = re.sub(r"infinite-spacing-height\s+=\s+##f",
                 r"extra-spacing-height = #'(0 . 0)", str)
    return str
...
def conv(str):
    str = re.sub(r'\\include(\s+)"gregorian-init.ly"',
                 r'\\include\1"gregorian.ly"', str)
    return str


@rule((2, 11, 62), "makam-init.ly -> makam.ly, \\bigger -> \\larger")
def conv(str):
    str = re.sub(r'\\include(\s+)"makam-init.ly"',
                 r'\\include\1"makam.ly"', str)
...
def conv(str):
    ...
    str = re.sub('next-staff-spacing',
                 'staff-staff-spacing',             str)
    # this is not a mistake:
    #   Both 'next- and 'between- become 'staff-staff-spacing.
    #   There is no conflict since they are in different grobs.
    str = re.sub('between-staff-spacing',
                 'staff-staff-spacing',             str)
    str = re.sub('after-last-staff-spacing',
                 'staffgroup-staff-spacing',        str)
    str = re.sub('inter-staff-spacing',
                 'nonstaff-relatedstaff-spacing',   str)
    str = re.sub('non-affinity-spacing',
                 'nonstaff-unrelatedstaff-spacing', str)
    str = re.sub('inter-loose-line-spacing',
                 'nonstaff-nonstaff-spacing',       str)
...
def conv(str):
    str = re.sub(
        r'\(space\s+\.\s+([0-9]*\.?[0-9]*)\)', r'(basic-distance . \1)', str)
    str = re.sub(r"#'space\s+=\s+#?([0-9]*\.?[0-9]*)",
                 r"#'basic-distance = #\1", str)
...
def conv(str):
    str = re.sub(
        r"\\(override|revert)TimeSignatureSettings(\s+[^#]*)(#[^#]*)#", r"\\\1TimeSignatureSettings\2#", str)
    return str
...
def conv(str):
    ...
    str = re.sub(r"(\s+(?:\\once\s*)?)\\override\s+Stem\s+#'transparent\s*=\s*##t",
                 r"\g<1>\\override Stem #'transparent = ##t\g<1>\\override Flag #'transparent = ##t", str)
    str = re.sub(r"(\s+(?:\\once\s*)?)\\revert\s*Stem\s+#'transparent",
                 r"\g<1>\\revert Stem #'transparent\g<1>\\revert Flag #'transparent", str)
    str = re.sub(r"(\s+(?:\\once\s*)?)\\override\s+Stem\s+#'stencil\s*=\s*##f",
                 r"\g<1>\\override Stem #'stencil = ##f\g<1>\\override Flag #'stencil = ##f", str)
    str = re.sub(r"(\s+(?:\\once\s*)?)\\revert\s*Stem\s+#'stencil",
                 r"\g<1>\\revert Stem #'stencil\g<1>\\revert Flag #'stencil", str)
    return str
...
def conv(str):
    ...
    str = re.sub('blank-after-score-page-force',
                 'blank-after-score-page-penalty', str)
    return str
...
def conv(str):
    ...
    m = re.search(
        r'\\language\s(?!\s*#?"(?:nederlands|deutsch|english|norsk|suomi|svenska))"', str)
    if m:
...
def conv(str):
    str = re.sub('OctavateEight',
                 'ClefModifier',                   str)
    str = re.sub('octavate-eight-interface',
                 'clef-modifier-interface',        str)
    str = re.sub('clefOctavation',
                 'clefTransposition',              str)
    str = re.sub('clefOctavationFormatter',
                 'clefTranspositionFormatter',     str)
    str = re.sub('clefOctavationStyle',
                 'clefTranspositionStyle',         str)
    str = re.sub('cueClefOctavation',
                 'cueClefTransposition',           str)
    str = re.sub('cueClefOctavationFormatter',
                 'cueClefTranspositionFormatter',  str)
    str = re.sub('cueClefOctavationStyle',
                 'cueClefTranspositionStyle',      str)
    return str
...
csharp -> c-sharp""")
def conv(str):
    str = re.sub(r'\bimplicitTimeSignatureVisibility\b',
                 'initialTimeSignatureVisibility', str)
...
def conv(str):
    ...
    str = re.sub(
        r":3\.5((?:\.[0-9]+)*\^?:[0-9]+(?:\.[0-9]+)*)\.3(?![.0-9])", r":5\1", str)
