...
$(outdir)/%.pfa: %.mf
	$(MFTRACE) $(MFTRACE_FLAGS) -I $(outdir)/ --formats=pfa --simplify --keep-trying $(basename $(@F))
	mv $(basename $(@F)).pfa $(outdir)

$(outdir)/%.pfb: %.mf
	$(MFTRACE)  $(MFTRACE_FLAGS) -I $(outdir)/ --formats=pfa,pfb --simplify --keep-trying  $(basename $(@F))
	-mv $(basename $(@F)).pfa $(outdir)
