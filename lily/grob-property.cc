...
print_property_callback_stack ()
{
  ...
  for (SCM s = grob_property_callback_stack; scm_is_pair (s); s = scm_cdr (s))
    message (
        _f ("%d: %s", frame++, ly_scm_write_string (scm_car (s)).c_str ()));
}
#endif
...
{
  return scm_list_3 (mutable_property_alist_, immutable_property_alist_, def);
}
...
Grob::internal_get_property (SCM sym) const
{
  ...
  if (scm_is_eq (val, ly_symbol2scm ("calculation-in-progress")))
    {
      programming_error (
          to_string ("cyclic dependency: calculation-in-progress encountered "
                     "for #'%s (%s)",
...
/* Unlike internal_get_property, this function does no caching. Use it,
...
SCM
Grob::internal_get_maybe_pure_property (SCM sym, bool pure, int start,
                                        int end) const
{
  ...
  if (scm_is_eq (value, SCM_UNSPECIFIED))
  ...
  else
    {
      ...
      if (ly_is_procedure (cache_callback))
        scm_call_4 (cache_callback, self_scm (), sym, proc, value);
#endif
  ...
  if (Unpure_pure_container *upc = unsmob<Unpure_pure_container> (value))
    {
      ...
      if (ly_is_procedure (value))
        return scm_apply_3 (value, scm_car (args), scm_from_int (start),
                            scm_from_int (end), scm_cdr (args));
