...
Score_performer::connect_to_context (Context *c)
{
  ...
  d->add_listener (GET_LISTENER (Score_performer, one_time_step),
                   ly_symbol2scm ("OneTimeStep"));
  d->add_listener (GET_LISTENER (Score_performer, prepare),
                   ly_symbol2scm ("Prepare"));
  d->add_listener (GET_LISTENER (Score_performer, finish),
...
Score_performer::disconnect_from_context ()
{
  ...
  d->remove_listener (GET_LISTENER (Score_performer, one_time_step),
                      ly_symbol2scm ("OneTimeStep"));
  d->remove_listener (GET_LISTENER (Score_performer, prepare),
                      ly_symbol2scm ("Prepare"));
  d->remove_listener (GET_LISTENER (Score_performer, finish),
                      ly_symbol2scm ("Finish"));
...
void Score_performer::finish (SCM)
{
...
void Score_performer::one_time_step (SCM)
{
