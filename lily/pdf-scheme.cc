/*
  ...
  Copyright (C) 2011--2022 Reinhold Kainhofer <reinhold@kainhofer.com>
...
that's not possible, as full UTF-16BE with a leading Byte Order Mark (BOM).
           )")
{
    ...
    if (!g) // ASCII not possible; resort to UTF-16BE
      {
        ...
        if (g_without_BOM) // conversion to UTF-16be might have failed (shouldn't!)
          {
            g = static_cast<char *> (g_malloc (bytes_written + 3));
