...
/*
  ...
  Let A_{k, n} = (a_{k, n, 1}, ... a_{k, n, k}) be the optimal set of line
  breaks for k systems and n potential breakpoints. a_{k, n, k} = n (it is the
  end of the piece)
...
Constrained_breaking::calc_subproblem (vsize start, vsize sys, vsize brk)
{
  ...
  for (vsize j = max_index; j-- > sys;)
    {
      if (0 == sys && j > 0)
        continue; /* the first line cannot have its first break after the
                     beginning */
...
{
  bool ragged_right
      = to_boolean (pscore_->layout ()->c_variable ("ragged-right"));
  bool ragged_last
      = to_boolean (pscore_->layout ()->c_variable ("ragged-last"));
                                         ...
                                         vsize end)
{
  ...
  out->last_column_ = c;
  out->break_penalty_
      = robust_scm2double (c->get_property ("line-break-penalty"), 0);
  out->page_penalty_
      = robust_scm2double (c->get_property ("page-break-penalty"), 0);
  out->turn_penalty_
      = robust_scm2double (c->get_property ("page-turn-penalty"), 0);
...
Line_details::Line_details (Prob *pb, Output_def *paper)
{
  ...
  break_penalty_ = 0;
  page_penalty_
      = robust_scm2double (pb->get_property ("page-break-penalty"), 0);
