...
One_line_auto_height_breaking::solve ()
{
  ...
  // and heights.  This would need support in the backends (eg.
  // framework-ps.scm).
  Real right_margin
  ...
  Real top_margin
      = robust_scm2double (book_->paper_->c_variable ("top-margin"), 0.0);
  Real bottom_margin
      = robust_scm2double (book_->paper_->c_variable ("bottom-margin"), 0.0);
