...
ADD_TRANSLATOR_GROUP (Score_engraver,
                      /* doc */
                      R"(
The top-level engraver.  Takes care of generating columns and the complete
system (i.e., @code{System}).
...
have a bar line at a certain point sets @code{forbidBreaks} to stop line
breaks.  In practice, this means that you can make a break point by creating a
bar line (assuming that there are no beams or notes that prevent a break
point).
                )",
