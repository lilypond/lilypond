      ...
      if (i)
      ...
      else
        {
          ...
          if (j)
            ...
            return Page_spacing_result (); /* couldn't salvage it -- probably
                                              going to crash */
        }
    }
...
// configuration for putting lines 0 through LINE-1 on PAGE+1 pages, provided
// that we have previously called calc_subproblem(page-1, k) for every k < LINE.
//
// This algorithm is similar to the constrained-breaking algorithm.
//
// If page == VPOS, we act on simple_state_ instead of state_.  This is useful
// if we don't want to constrain the number of pages that the solution has.  In
// this case, the algorithm looks more like the page-turn-page-breaking
// algorithm.  But the subproblems look similar for both, so we reuse this
// method.
bool
Page_spacer::calc_subproblem (vsize page, vsize line)
{
  ...
  // Note: if page == VPOS then we don't actually know yet which page number
  // we're working on, so we have to recalculate the page height in the loop.
  // Therefore our early-exit condition from the loop depends on paper_height
  // rather than page_height (ie. we break only if we would overfill a page
  // without margins or headers/footers).  Otherwise, the algorithm would not be
  // optimal: if our page has a very large header then perhaps we should look
  // ahead a few systems in order to find the best solution.  A good example of
  // this is input/regression/page-spacing-tall-headfoot.ly
  ...
  for (vsize page_start = line + 1; page_start > page_num && page_start--;)
    {
      ...
      // This 'if' statement is a little hard to parse. It won't consider this
      // configuration if it is overfull unless the current configuration is the
      // first one with this start point. We also make an exception (and
      // consider this configuration) if the previous configuration we tried had
      // fewer lines than min-systems-per-page.
      ...
      if (page > 0 || page_start == 0)
        {
          // If the last page is ragged, set its force to zero. This way, we
          // will leave the last page half-empty rather than trying to balance
          // things out (which only makes sense in non-ragged situations).
