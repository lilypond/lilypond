      ...
      if (ret.size () && !scm_is_symbol (ret.back ().page_permission_))
        {
          ...
          /*
            We must account for the padding between the lines that we are
            compressing. The padding values come from "old," which is the upper
            system here. Note the meaning of tight-spacing: if a system has
            tight-spacing, then the padding _before_ it is ignored.
          */
          Real padding = 0;
                              ...
                              Prob_break_predicate prob_break)
{
  ...
  system_count_ = 0;
  paper_height_
      = robust_scm2double (pb->paper_->c_variable ("paper-height"), 1.0);
  ...
  systems_per_page_ = std::max (
      0, robust_scm2int (pb->paper_->c_variable ("systems-per-page"), 0));
  max_systems_per_page_ = std::max (
      0, robust_scm2int (pb->paper_->c_variable ("max-systems-per-page"), 0));
  min_systems_per_page_ = std::max (
      0, robust_scm2int (pb->paper_->c_variable ("min-systems-per-page"), 0));
  orphan_penalty_
      = robust_scm2int (pb->paper_->c_variable ("orphan-penalty"), 100000);

  Stencil footnote_separator
      = Page_layout_problem::get_footnote_separator_stencil (pb->paper_);
  ...
  in_note_padding_
      = robust_scm2double (pb->paper_->c_variable ("in-note-padding"), 0.0);
  footnote_footer_padding_ = robust_scm2double (
      pb->paper_->c_variable ("footnote-footer-padding"), 0.0);

  footnote_number_raise_ = robust_scm2double (
      pb->paper_->c_variable ("footnote-number-raise"), 0.0);
...
Page_breaking::make_page (int page_num, bool last) const
{
  ...
  return scm_apply_0 (
      make_page_scm,
      scm_list_n (book_->self_scm (), ly_symbol2scm ("page-number"),
                  scm_from_int (page_num), ly_symbol2scm ("is-last-bookpart"),
...
SCM
Page_breaking::get_page_configuration (SCM systems, int page_num, bool ragged,
                                       bool last)
{
...
SCM
Page_breaking::draw_page (SCM systems, SCM configuration, int page_num,
                          bool last)
{
...
Page_breaking::make_pages (const vector<vsize> &lines_per_page, SCM systems)
{
  ...
  SCM ret = SCM_EOL;
  bool reset_footnotes_on_new_page = to_boolean (
  ...
  for (vsize i = 0; i < page_count; i++)
    {
      ...
      systems_configs_fncounts
          = scm_cons (scm_cons (lines, config), systems_configs_fncounts);
      ...
      for (SCM l = lines; scm_is_pair (l); l = scm_cdr (l))
        {
          ...
          for (SCM lbls = labels; scm_is_pair (lbls); lbls = scm_cdr (lbls))
            label_page_table = scm_cons (
                scm_cons (scm_car (lbls), page_num_scm), label_page_table);
        }
      ...
      if (system_specs_[i].pscore_)
        {
          ...
          SCM system_count = system_specs_[i].pscore_->layout ()->c_variable (
              "system-count");
          if (scm_is_number (system_count))
            {
              ...
              vector<Line_details> details
                  = breaking.line_details (0, VPOS, scm_to_int (system_count));
          ...
          for (vsize j = 0; j < cols.size (); j++)
            {
              ...
              bool chunk_end = scm_is_eq (
                  cols[j]->get_property ("page-break-permission"), force_sym);
...
Page_breaking::blank_page_penalty () const
{
  ...
  if (Paper_score *ps = system_specs_[pos.system_spec_index_].pscore_)
    return robust_scm2double (ps->layout ()->lookup_variable (penalty_sym),
                              0.0);
    ...
    Real penalty_for_fewer_pages)
{
  ...
  Real page_spacing_weight = robust_scm2double (
      book_->paper_->c_variable ("page-spacing-weight"), 10);
      ...
      if (line == cached_line_details_.size () - 1)
        {
           ...
           * last system is moved to a new page */
          space.resize (page_height (page_num, true));
                                        ...
                                        Page_spacing_result res)
{
  ...
  Real page_weighting = robust_scm2double (
      book_->paper_->c_variable ("page-spacing-weight"), 10);
                                        ...
                                        Real page_height, bool ragged)
{
  ...
  /* don't do finalize_spacing_result () because we are only an internal
   * function */
  return ret;
}
                                         ...
                                         int first_page_num)
{
  ...
  // page1_status and page2_status keep track of whether the
  // min-systems-per-page and max-systems-per-page constraints are satisfied.
  ...
  for (vsize i = 0; i < page1_force.size (); i++)
    {
      ...
      if (ragged1)
        page2_force[page2_force.size () - 1 - i]
            = (page2.force_ < 0 && i + 1 < page1_force.size ()) ? infinity_f
                                                                : 0;
