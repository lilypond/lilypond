...
@iref{Ambitus}.")))))
    ...
    (AmbitusNoteHead
     . (
        (duration-log . 2)
        (glyph-name . ,note-head::get-glyph-name)
        (ignore-ambitus . #t)
...
line to visually mark and annotate another grob.")))))
    ...
    (BarLine
     . (
        ;;
        ;; Ross. page 151 lists other values for `kern`, `segno-kern`, and
        ;; `hair-thickness`; we opt for a leaner look
        ;;
        ;; TODO:
...
passage.")))))
    ...
    (StaffSymbol
     . (
        ...
        (line-count . 5)
        (line-positions . ,ly:staff-symbol::calc-line-positions)
...
start delimiter.")))))
    ...
    (TabNoteHead
     . (
        (after-line-breaking . ,tab-note-head::handle-ties)
        (bend-me . ())
        (details . ((cautionary-properties . ((angularity . 0.4)
                    ...
                    (harmonic-properties . ((angularity . 2)
                                            ...
                                            (width . 0.25)))
                    (head-offset . 3/5)
                    (tied-properties . ((note-head-visible . #t)
                                        (parenthesize . #t)
                                        (repeat-tied . #f)
                                        (tied . #f)))))
        (direction . ,CENTER)
...
direction and shape of stacked @iref{Tie} grobs.")))))
    ...
    (TimeSignature
     . (
        ...
        (fraction . ,ly:time-signature::calc-fraction)
...
of a voice.")))))
    ...
    (VoltaBracket
     . (
        ...
        (text . ,volta-bracket-interface::calc-text)
