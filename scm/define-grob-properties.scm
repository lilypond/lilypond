...
automatic beam-collision resolution is run.")
     (collision-voice-only ,boolean? "Avoid beam collisions only with
...
;;;
     ...
     (gap-count ,integer? "Number of @q{floating} beams in a two-stem
tremolo.")
...
@code{(@var{x-start} @var{y-start} @var{x-end} @var{y-end})}.")
     (measure-length ,positive-musical-length-as-moment? "Length of a measure.
...
Y@tie{}dimension, measured in @code{staff-space} units.")
     (musical-length ,musical-length-as-moment? "Musical length.")
...
dot.")
     (protrusion ,number? "In a bracket indicating non-arpeggio or optional
...
Usually @code{#f} by default.  If @code{whiteout-color} is set, use
this color instead of white for the background.")
     (whiteout-color ,color? "The background color used if property
@code{whiteout} is set.")
