...
%
\def\texinfoversion{2025-02-07.09}
%
...
\def\scanmacro#1{%
  \newlinechar`\^^M
  % expand \eatleadingcr three times to maybe remove a leading
  % newline, then call \eatspaces on the result.
  \def\xeatspaces##1{%
...
% Trim a single leading ^^M off a string.
{\catcode`\^^M=\active \catcode`\Q=3%
\gdef\eatleadingcr #1{\eatlcra Q#1Q^^MQ}%
\gdef\eatlcra #1Q^^M{\eatlcrb #1Q}%
\gdef\eatlcrb Q#1Q#2Q{#1}%
}
...
\def\anchor#1{\savesf \setref{#1}{Yanchor}\restoresf \ignorespaces}
...
\def\getprintedrefname#1#2#3{%
  ...
  \ifdim \wd\printedrefnamebox = 0pt
    ...
    \else
      ...
      \else
        \getrefx{#1-snt}\tmp
        \ifx\tmp\Yanchor % reference to anchor
          \def\printedrefname{\ignorespaces #1}%
        \else
          \ifhavexrefs
