...
\header {
  texidoc = "@code{\set} works on a deprecated context property.  The output
should be a single measure with a B@tie{}note."
}
...
#(ly:expect-warning
  ...
  "deprecatedLySet" "skipTypesetting")
...
#(define-deprecated-property
  'translation-type? 'deprecatedLySet boolean?
  #:new-symbol 'skipTypesetting
  #:old->new not)

\fixed c' {
  \set Timing.deprecatedLySet = ##f
  R1^"FAIL"
  %% We set the deprecated property again, but lilypond warns only once.
  \set Timing.deprecatedLySet = ##t
  b1
}
